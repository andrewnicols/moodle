...
import {getOwningComponentDirectory, fetchComponentData} from "../../.grunt/components.js";
...
/**
 * Translate an absolute source file path into its `@moodle/lms/<component>/<module>`
 * alias specifier, the same specifier a hand-written cross-component import would use.
 *
 * Handles theme override files specially: a file under
 * `<theme>/js/esm/src/overrides/<component>/*` is addressed via the
 * `@moodle/lms/theme-<theme>/<component>/*` alias rather than the theme's own
 * component name, matching the mapping generated by generate-aliases.mjs.
 *
 * @param {string} resolved Absolute path to the target source file.
 * @returns {string|null} The `@moodle/lms/...` specifier (no file extension, matching the
 *          convention used by hand-written cross-component imports), or null
 *          if the file isn't owned by a known component or isn't inside its `esm/src` tree.
 */
function resolveAbsoluteSpecifier(resolved) {
    const ownerDir = getOwningComponentDirectory(resolved);
    if (!ownerDir) {
        return null;
    }

    const componentName = fetchComponentData().components[ownerDir];
    if (!componentName) {
        return null;
    }

    // Theme override files live under <themeDir>/js/esm/src/overrides/<component>/*
    // and are addressed via the theme-specific alias for the overridden component,
    // not the theme's own component name.
    if (componentName.startsWith('theme_')) {
        const overridesRoot = path.join(projectRoot, ownerDir, 'js', 'esm', 'src', 'overrides');
        const relFromOverrides = path.relative(overridesRoot, resolved);
        if (relFromOverrides && !relFromOverrides.startsWith('..')) {
            const [overriddenComponent, ...rest] = relFromOverrides.split(path.sep);
            const themeName = componentName.replace(/^theme_/, '');
            const subpath = rest.join('/').replace(/\.(ts|tsx|js|jsx)$/, '');
            return `@moodle/lms/theme-${themeName}/${overriddenComponent}/${subpath}`;
        }
    }

    const srcRoot = path.join(projectRoot, ownerDir, 'js', 'esm', 'src');
    const rel = path.relative(srcRoot, resolved);
    if (rel.startsWith('..')) {
        return null;
    }

    const subpath = rel.replace(/\\/g, '/').replace(/\.(ts|tsx|js|jsx)$/, '');
    return `@moodle/lms/${componentName}/${subpath}`;
}

/**
 * An esbuild plugin that translates relative imports within esm/src trees into their
 * absolute `@moodle/lms/<component>/<module>` alias equivalent.
 *
 * A relative import (e.g. `./stringUtils` or `../src/views/ActivityIcon`) must behave
 * identically to the equivalent absolute alias import (e.g.
 * `@moodle/lms/core/stringUtils`): both are resolved by the browser's import map at
 * runtime, and both must be recognisable as `@moodle/lms/...` specifiers by tooling such
 * as the component swizzling system, which rewrites/redirects imports by specifier and
 * does not follow relative filesystem paths. Left as a relative specifier, an import
 * would bypass the import map entirely (relative specifiers are resolved directly as a
 * URL, never intercepted by the import map) and would be invisible to swizzling.
 *
 * Note: onResolve hooks are only invoked by esbuild when `bundle: true`, so this plugin
 * requires bundling to be enabled even though no code is actually inlined (every resolved
 * import is marked `external`).
 *
    ...
    return {
        ...
        setup(build) {
            ...
            build.onResolve({ filter: /^\.\.?\// }, (args) => {
                // Only translate imports within esm/src trees.
                if (!args.importer || !args.importer.includes(path.join('esm', 'src'))) {
                    return null;
                }

                // Resolve the full path to determine if it's within an esm/src tree.
                const resolved = path.resolve(path.dirname(args.importer), args.path);
                if (!resolved.includes(path.join('esm', 'src'))) {
                    return null;
                }

                const specifier = resolveAbsoluteSpecifier(resolved);
                if (!specifier) {
                    // Could not map to a component alias (e.g. unknown owning component) —
                    // fall back to a browser-resolvable relative URL pointing at the built file.
                    const externalPath = args.path.replace(/\.(ts|tsx|js|jsx)$/, '') + '.js';
...
/**
 * An esbuild plugin that externalizes every non-relative (bare specifier) import,
 * leaving it untouched in the output.
 *
 * Bare specifiers (e.g. `react`, `@moodle/lms/core/config`) are resolved at runtime via
 * the browser's import map, not by esbuild. Since `bundle: true` is required for the
 * relative-import rewriting above (see externalRelativeImports()), esbuild would
 * otherwise try to actually resolve and inline these packages (or fail outright for
 * aliases like `@moodle/lms/*` that don't exist in node_modules). Marking them external
 * unchanged preserves the previous `bundle: false` behaviour for bare specifiers.
 *
 * @returns {import('esbuild').Plugin}
 */
function externalBareImports() {
    return {
        name: 'external-bare-imports',
        setup(build) {
            // Match everything that isn't a relative or absolute filesystem path.
            build.onResolve({ filter: /^[^./]/ }, (args) => {
...
function createBuildConfig() {
    return {
        // `bundle: true` is required so esbuild's onResolve plugins run at all (they are
        // never invoked when `bundle: false`). Nothing is actually inlined though: every
        // resolved import (relative or bare) is marked `external` by the plugins below,
        // so each source file is still transformed independently, one-to-one.
        bundle: true,
        format: "esm",
        jsx: "automatic",
        treeShaking: true,
        plugins: [configPlugin(), externalRelativeImports(), externalBareImports()],
