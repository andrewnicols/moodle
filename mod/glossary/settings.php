...
if ($ADMIN->fulltree) {
    ...
    if (empty($CFG->enablerssfeeds)) {
        ...
        $str = get_string('configenablerssfeeds', 'mod_glossary').'<br />'.get_string('configenablerssfeedsdisabled2', 'core_admin');

    } else {
        $options = array(0=>get_string('no'), 1=>get_string('yes'));
        $str = get_string('configenablerssfeeds', 'mod_glossary');
    }
    $settings->add(new admin_setting_configselect('glossary_enablerssfeeds', get_string('enablerssfeeds', 'core_admin'),
                       $str, 0, $options));
