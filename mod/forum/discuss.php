...
if ($move > 0 && confirm_sesskey()) {
    ...
    if (!$capabilitymanager->can_move_discussions($USER)) {
        if ($forum->get_type() == 'single') {
            ...
            throw new \moodle_exception('nopermissions', 'mod_error', $return, get_capability_string('mod/forum:movediscussions'));
        }
    }
...
if ($pin !== -1 && confirm_sesskey()) {
    if (!$capabilitymanager->can_pin_discussions($USER)) {
        throw new \moodle_exception('nopermissions', 'mod_error', $return, get_capability_string('mod/forum:pindiscussions'));
    }
