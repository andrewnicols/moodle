    ...
    function definition() {
        ...
        if (!$inpagereply) {
            ...
            if (\mod_forum\subscriptions::is_forcesubscribed($forum)) {
                ...
                $mform->addHelpButton('discussionsubscribe', 'forcesubscribed', 'mod_forum');

            } else if (\mod_forum\subscriptions::subscription_disabled($forum) && !$manageactivities) {
                $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'mod_forum'));
                $mform->freeze('discussionsubscribe');
                $mform->setDefaults('discussionsubscribe', 0);
                $mform->addHelpButton('discussionsubscribe', 'disallowsubscription', 'mod_forum');

            } else {
                $mform->addElement('checkbox', 'discussionsubscribe', get_string('discussionsubscription', 'mod_forum'));
                $mform->addHelpButton('discussionsubscribe', 'discussionsubscription', 'mod_forum');
            }
            ...
            if (forum_can_create_attachment($forum, $modcontext)) {
                ...
                $mform->addHelpButton('attachments', 'attachment', 'mod_forum');
            }
            ...
            if (!$post->parent && has_capability('mod/forum:pindiscussions', $modcontext)) {
                ...
                $mform->addHelpButton('pinned', 'discussionpinned', 'mod_forum');
            }
            ...
            if ((empty($post->id) && $canreplyprivately) || (!empty($post) && !empty($post->privatereplyto))) {
                ...
                $mform->addHelpButton('isprivatereply', 'privatereply', 'mod_forum');
            ...
            if ($groupmode = groups_get_activity_groupmode($cm, $course)) {
                ...
                if ($canposttoowngroups) {
                    ...
                    $mform->addHelpButton('posttomygroups', 'posttomygroups', 'mod_forum');
            ...
            if (!empty($CFG->forum_enabletimedposts) && !$post->parent &&
                ...
                $mform->addElement('date_time_selector', 'timestart', get_string('displaystart', 'mod_forum'),
                    array('optional' => true));
                $mform->addHelpButton('timestart', 'displaystart', 'mod_forum');

                $mform->addElement('date_time_selector', 'timeend', get_string('displayend', 'mod_forum'),
                    array('optional' => true));
                $mform->addHelpButton('timeend', 'displayend', 'mod_forum');
