...
M.util.get_string = function(identifier, component, a) {
    ...
    if (!M.str.hasOwnProperty(component) || !M.str[component].hasOwnProperty(identifier)) {
        ...
        window.console.warn(`Undefined string '${stringvalue}`, 'M.util.get_string');
        return stringvalue;
    }
    ...
    if (typeof a == 'object') {
        ...
        for (var key in a) {
            if (typeof a[key] != 'number' && typeof a[key] != 'string') {
                window.console.warn(`Invalid value type for $a->${key}`, 'M.util.get_string');
                continue;
            }
            var search = '{$a->' + key + '}';
    ...
    window.console.warn('Incorrect placeholder type', 'M.util.get_string');
    return stringvalue;
};
