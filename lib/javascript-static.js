...
// Create the pendingJS history in the current `M`.
M.util.pending_js = [];
M.util.complete_js = [];

let topWindow = window;
if (window !== window.parent) {
    if (window.parent.M && window.parent.M.util && window.parent.M.util.pending_js) {
 ...
 */
M.util.js_pending = uniqid => {
    if (typeof uniqid !== 'undefined') {
        topWindow.M.util.pending_js.push(uniqid);
    }

    return topWindow.M.util.pending_js.length;
};
 ...
 */
M.util.js_complete = uniqid => {
    const index = topWindow.M.util.pending_js.indexOf(uniqid);
    if (index >= 0) {
        topWindow.M.util.complete_js.push(
            topWindow.M.util.pending_js.splice(index, 1)
        );
    } else {
