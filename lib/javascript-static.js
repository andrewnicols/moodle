 ...
 */
M.util.show_confirm_dialog = function(e, {
    message,
    callback = null,
    scope = null,
    callbackargs = [],
    ...
    require(['core/notification', 'core/str', 'core/normalise'], function(Notification, Str, Normalise) {
        var target = Normalise.getNode(e.target);
        ...
        const confirmationCallback = () => {
            ...
            const ancestorLink = target.closest('a');
            if (ancestorLink) {
                window.location = ancestorLink.href;
                return;
            }

            if (target.matches('input') || target.matches('button')) {
                if (!target.form) {
                    return;
                }
                ...
                if (target.hasAttribute('value')) {
                    ...
                    hiddenValue.name = target.name;
                    hiddenValue.value = target.value;
                    target.form.appendChild(hiddenValue);
                    FormChangeChecker.markFormAsDirty(target.form);
                }
                target.form.submit();
                return;
            }
            ...
            window.console.warn(`Element of type ${target.tagName} is not supported.`);
        };
