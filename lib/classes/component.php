            ...
            if (is_array($legacyclasses)) {
                foreach ($legacyclasses as $classname => $path) {
                    if (is_array($path)) {
                        if (!$allowsubsystems) {
                            throw new \Exception(
                                "Invalid legacy classes path entry for {$classname}. Only files within the component can be specified.",
                            );
                        }
                        if (count($path) !== 2) {
                            throw new \Exception(
                                "Invalid legacy classes path entry for {$classname}. Entrist must be in the format [subsystem, path].",
                            );
                        }
                        [$subsystem, $path] = $path;
