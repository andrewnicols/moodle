...
class global_navigation extends node {
    ...
    /** @var null|bool A switch for whether or not to show categories in the my courses branch. */
    ...
    public function initialise() {
        ...
        $this->rootnodes['myprofile'] = $this->add(
            get_string('profile'),
            null,
            self::TYPE_USER,
            null,
        ...
        // Set $issite to true if the current pages course is the sites frontpage course.
        ...
        if (
            ...
            && $this->page->context->instanceid != $SITE->id && $canviewcourseprofile
        ) {
    ...
    protected function load_all_courses($categoryids = null) {
        ...
        if ($this->show_categories()) {
            ...
            // We now need to fetch up to $limit courses for each category to e displayed.
            if ($categoryids !== null) {
            ...
            if (count($fullfetch)) {
                ...
                foreach ($coursesrs as $course) {
                    ...
                    if (!$this->should_add_course_to_navigation($course)) {
                        continue;
                    }
    ...
    protected function load_all_categories($categoryid = self::LOAD_ROOT_CATEGORIES, $showbasecategories = false) {
        ...
        $categoriestoload = [];
        if ($categoryid == self::LOAD_ALL_CATEGORIES) { // phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedIf
    ...
    public function load_generic_course_sections(stdClass $course, node $coursenode) {
        ...
        foreach ($sections as $sectionid => $section) {
            ...
            if ($course->id == $SITE->id) {
                ...
                if (
                    !$section->uservisible || (!$this->showemptysections &&
     ...
     * @return null|node or null if not accessible
     */
    ...
    protected function load_for_user($user = null, $forceforcontext = false) {
        ...
        if ($issitecourse) {
            ...
            if (!empty($CFG->navadduserpostslinks)) {
                ...
                $forumtab->add(get_string('discussions', 'forum'), new url(
                    '/mod/forum/user.php',
    ...
    public function add_course(stdClass $course, $forcegeneric = false, $coursetype = self::COURSE_OTHER) {
        ...
        if ($coursetype != self::COURSE_MY && $coursetype != self::COURSE_CURRENT && $course->id != $SITE->id) {
            if (is_role_switched($course->id)) { // phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedIf
    ...
    protected function is_category_fully_loaded($categoryid) {
        return (
            ...
            && (
                $this->allcategoriesloaded || $this->addedcategories[$categoryid]->children->count() > 0
            )
        );
    }
    ...
    public function add_course_essentials($coursenode, stdClass $course) {
        ...
        if ($navoptions->badges) {
            $url = new url('/badges/view.php', ['type' => 2, 'id' => $course->id]);
    ...
    public function add_front_page_course_essentials(node $coursenode, stdClass $course) {
        ...
        // Badges.
        if ($navoptions->badges) {
            $url = new url($CFG->wwwroot . '/badges/view.php', ['type' => 1]);
    ...
    /**
     * Whether the course shoudl be added to navigation.
     *
     * @param stdClass $course
     * @return bool
     */
    private function should_add_course_to_navigation(stdClass $course): bool {
        if ($course->visible) {
            // The course is visible already.
            return true;
        }

        if (is_role_switched($course->id)) {
            // The role is switched for this user/course combination.
            // The course node needs to be visible to return to the original role.
            return false;
        }

        // The course is hidden, but the user has the capability to view hidden courses in this context.
        return has_capability('moodle/course:viewhiddencourses', context_course::instance($course->id));
    }
}
