 ...
 */
class settings_navigation extends node {
    /** @var context the current context */
    protected $context;
    /** @var moodle_page the moodle page that the navigation belongs to */
    protected $page;
    /** @var string contains administration section nodes */
     ...
     *
     * It does this by first calling the parent's add method {@link node::add()}
     ...
     * @param string|int $key a key to access the node by.
     * @param pix_icon $icon An icon that appears next to the node.
     * @return node $node with the new node added to it.
     */
     ...
     *
     * @param node $referencebranch A reference to a branch in the settings
     *      navigation tree
     ...
     */
    protected function load_administration_settings(node $referencebranch = null, part_of_admin_tree $adminbranch = null) {
        global $CFG;
        ...
        if ($referencebranch === null) {
            ...
            $reference = $referencebranch->add(
                text: $adminbranch->visiblename,
                action: $url,
                type: self::TYPE_SETTING,
                key: $adminbranch->name,
                icon: $icon,
            );

            if ($adminbranch->is_hidden()) {
                if (
                    ($adminbranch instanceof admin_externalpage || $adminbranch instanceof admin_settingpage)
     ...
     * @param node $node
     */
    protected function scan_for_active_node(node $node) {
    ...
    protected function load_course_settings($forceopen = false) {
        ...
        if ($adminoptions->editcompletion) {
            ...
            $coursenode->add(
                text: get_string('coursecompletion', 'completion'),
                action: $url,
                type: self::TYPE_SETTING,
        ...
        if (!$adminoptions->update && $adminoptions->tags) {
            $url = new url('/course/tags.php', ['id' => $course->id]);
            $coursenode->add(
                text: get_string('coursetags', 'tag'),
                action: $url,
                type: self::TYPE_SETTING,
                key: 'coursetags',
                icon: new pix_icon('i/settings', ''),
        ...
        if ($adminoptions->filters) {
            ...
            $coursenode->add(
                text: get_string('filters', 'admin'),
                action: $url,
                type: self::TYPE_SETTING,
                key: 'filtermanagement',
                icon: new pix_icon('i/filter', ''),
            );
        }
        ...
        if ($adminoptions->reports) {
            $reportnav = $coursenode->add(
                text: get_string('reports'),
                action: new url('/report/view.php', ['courseid' => $coursecontext->instanceid]),
        ...
        if ($adminoptions->gradebook) {
            ...
            $coursenode->add(
                text: get_string('gradebooksetup', 'grades'),
                action: $url,
                type: self::TYPE_SETTING,
                key: 'gradebooksetup',
                icon: new pix_icon('i/settings', ''),
        ...
        if ($adminoptions->outcomes) {
            ...
            $coursenode->add(
                text: get_string('outcomes', 'grades'),
                action: $url,
                type: self::TYPE_SETTING,
                key: 'outcomes',
        ...
        if ($adminoptions->update) {
            ...
            if ($haseditabletypes) {
                ...
                $coursenode->add(
                    text: get_string('repositories'),
                    action: $url,
                    type: self::TYPE_SETTING,
                    icon: new pix_icon('i/repository', ''),
                );
            }
        }
        ...
        if ($adminoptions->files) {
            ...
            $coursenode->add(
                text: get_string('courselegacyfiles'),
                action: $url,
                type: self::TYPE_SETTING,
                key: 'coursefiles',
    ...
    protected function load_module_settings() {
        ...
        $modulenode = $this->add(
            text: get_string('pluginadministration', $this->page->activityname),
            type: self::TYPE_SETTING,
            key: 'modulesettings',
        );
        $modulenode->nodetype = node::NODETYPE_BRANCH;
        ...
        if (count(get_assignable_roles($this->page->cm->context)) > 0) {
            ...
            $modulenode->add(
                text: get_string('localroles', 'role'),
                action: $url,
                type: self::TYPE_SETTING,
                key: 'roleassign',
                icon: new pix_icon('i/role', ''),
            );
        }
        // Override roles.
        if (
            ...
            $url = new url('/' . $CFG->admin . '/roles/permissions.php', ['contextid' => $this->page->cm->context->id]);
            $modulenode->add(
                text: get_string('permissions', 'role'),
                action: $url,
                type: self::TYPE_SETTING,
                key: 'roleoverride',
        ...
        $capabiltiies = ['moodle/role:assign', 'moodle/role:safeoverride', 'moodle/role:override', 'moodle/role:assign'];
        if (has_any_capability($capabiltiies, $this->page->cm->context)) {
            $url = new url('/' . $CFG->admin . '/roles/check.php', ['contextid' => $this->page->cm->context->id]);
            $modulenode->add(
                text: get_string('checkpermissions', 'role'),
                action: $url,
                type: self::TYPE_SETTING,
                key: 'rolecheck',
                icon: new pix_icon('i/checkpermissions', ''),
            );
        }
        ...
        if (function_exists($featuresfunc) && $featuresfunc(FEATURE_BACKUP_MOODLE2)) {
            if (has_capability('moodle/backup:backupactivity', $this->page->cm->context)) {
            ...
            if (has_capability('moodle/restore:restoreactivity', $this->page->cm->context)) {
        ...
        if (function_exists($featuresfunc) && $featuresfunc(FEATURE_ADVANCED_GRADING)) {
            if (has_capability('moodle/grade:managegradingforms', $this->page->cm->context)) {
        ...
        try {
            ...
            if ($usercanshare && $isvalidinstance) {
                ...
                $modulenode->add(
                    text: get_string('moodlenet:sharetomoodlenet', 'moodle'),
                    action: $action,
                    type: self::TYPE_SETTING,
                    key: 'exportmoodlenet',
                )->set_force_into_more_menu(true);
    ...
    protected function generate_user_settings($courseid, $userid, $gstitle = 'usercurrentsettings') {
        ...
        if ($gstitle == 'usercurrentsettings') {
            ...
            if (!empty($CFG->navadduserpostslinks)) {
                ...
                $forumtab->add(get_string('posts', 'forum'), new url('/mod/forum/user.php', $baseargs), null, 'myposts');
            ...
            if (!empty($CFG->enableblogs)) {
                ...
                if (count($options) > 0) {
                    $blogs = $profilenode->add(get_string('blogs', 'blog'), null, node::TYPE_CONTAINER);
            ...
            $usersetting = node::create(get_string('preferences', 'moodle'), $prefurl, self::TYPE_CONTAINER, null, $key);
        ...
        if (isloggedin() && !isguestuser($user)) {
            if (
                ...
                $useraccount->add(
                    text: get_string('calendarpreferences', 'calendar'),
                    action: $url,
                    type: self::TYPE_SETTING,
                    key: 'preferredcalendar',
                );
            }
        }
        ...
        if (isloggedin() && !isguestuser($user)) {
            if (
                ...
                $useraccount->add(
                    text: get_string('contentbankpreferences', 'core_contentbank'),
                    action: $url,
                    type: self::TYPE_SETTING,
                );
            }
        ...
        if (
            has_any_capability(['moodle/role:assign', 'moodle/role:safeoverride', 'moodle/role:override',
        ...
        if ($currentuser && !empty($CFG->enableblogs)) {
            $blog = $usersetting->add(get_string('blogs', 'blog'), null, node::TYPE_CONTAINER, null, 'blogs');
        ...
        if ($currentuser && !empty($CFG->enablebadges)) {
            $badges = $usersetting->add(get_string('badges'), null, node::TYPE_CONTAINER, null, 'badges');
    ...
    protected function load_block_settings() {
        ...
        if (has_capability('moodle/role:review', $this->context) || count(get_overridable_roles($this->context)) > 0) {
            $url = new url('/' . $CFG->admin . '/roles/permissions.php', ['contextid' => $this->context->id]);
            $blocknode->add(
        ...
        // Check role permissions.
        $capabilities = ['moodle/role:assign', 'moodle/role:safeoverride', 'moodle/role:override', 'moodle/role:assign'];
    ...
    protected function load_category_settings() {
        ...
        if (!empty($assignableroles)) {
            ...
            $categorynode->add(
                ...
                type: self::TYPE_SETTING,
                key: 'roles',
                icon: new pix_icon('i/assignroles', ''),
            );
        }
        ...
        if (has_capability('moodle/restore:restorecourse', $catcontext)) {
            ...
            $categorynode->add(
                text: get_string('restorecourse', 'admin'),
                action: $url,
                type: self::TYPE_SETTING,
                key: 'restorecourse',
                icon: new pix_icon('i/restore', ''),
    ...
    protected function load_front_page_settings($forceopen = false) {
        ...
        // Manage files.
        if ($adminoptions->files) {
            // Hidden in new installs.
            $url = new url('/files/index.php', ['contextid' => $coursecontext->id]);
            $frontpage->add(
                text: get_string('sitelegacyfiles'),
                action: $url,
                type: self::TYPE_SETTING,
                icon: new pix_icon('i/folder', ''),
            );
        ...
        if ($adminoptions->backup || $adminoptions->restore) {
            $coursereusenav = $frontpage->add(
                ...
                action: new url('/backup/view.php', ['id' => $course->id]),
                type: self::TYPE_CONTAINER,
                key: 'coursereuse',
