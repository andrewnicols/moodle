 ...
 * Bootstrap helper.
 *
 * Note: This class is not guaranteed to have class auotloading available.
 *
 * @package    core
 ...
 */
class bootstrap {
    protected const BEFORE_SETUP = 0;
    protected const AFTER_EARLY = 1;
    protected const AFTER_SETUP = 2;

    protected static int $stage = 0;

    public static function early_setup(): void {
        global $CFG, $DB, $PAGE, $OUTPUT;
        if (self::$stage >= self::AFTER_EARLY) {
            return;
        }
        ...
        self::check_cli_maintenance_mode();
        ...
        // Add pear include path to Moodle's lib/pear so that includes and requires will search there for files before anywhere else
    ...
    public static function full_setup(): void {
        self::early_setup();
        if (self::$stage >= self::AFTER_SETUP) {
            return;
        }
    ...
    protected static function setup_dirs(): void {
        global $CFG;
    ...
    protected static function behat(): void {
        global $CFG;
    ...
    protected static function phpunit(): void {
        global $CFG, $DB;
    ...
    protected static function setup_vars(): void {
        global $CFG;
        ...
        if (!defined('MOODLE_INTERNAL')) {
            // Necessary because cli installer has to define it earlier.
            /** Used by library scripts to check they are being called by Moodle. */
            define('MOODLE_INTERNAL', true);
        }
    }

    protected static function setup_xhprof(): void {
        global $CFG;
    ...
    protected static function setup_default_constants(): void {
    ...
    protected static function check_cli_maintenance_mode(): void {
        global $CFG;
    ...
    protected static function late_setup(): void {
        global $CFG;
        ...
        if (!empty($CFG->allowthemechangeonurl) and !empty($_GET['theme'])) {
            ...
            try {
                $themeconfig = \theme_config::load($urlthemename);
        ...
        if ($forcelang = optional_param('forcelang', '', PARAM_SAFEDIR)) {
            if (
                ...
                && has_capability('moodle/site:forcelanguage', \core\context\system::instance())
            ) {
