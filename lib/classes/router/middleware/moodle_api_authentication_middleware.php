...
class moodle_api_authentication_middleware extends moodle_authentication_middleware {
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface {
        ...
        // See if the Request is an OAuth2 Request.
        $authorisations = $request->getHeader('Authorization');
        if (count($authorisations) === 1) {
            $authorisation = $authorisations[0];

            if (str_starts_with($authorisation, 'Bearer')) {
                // If using Bearer Authentication itthe header is in the format:
                // Authorization: Bearer <token>.
                // Extract the Authorisation Bearer token.
                $token = substr($authorisation, 7);

                // Check the token against the database.
            }
        } else if ($authorisations) {
            // If there are multiple Authorisation headers, we cannot proceed.
            // We should throw an exception here.
            throw new \Exception('Multiple Authorisation headers found in request.');
        } else {
            array_key_exists('MoodleSession', $request->getCookieParams());
        }
