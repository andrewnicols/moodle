...
describe('Str.get_string', () => {
    describe('without a LocalStorage cache', () => {
        ...
        it('will fetch a string if not defined', async () => {
            // This test is testing the standard string loader so we do not want to replace the strong loader.
            global.restoreDefaultStringLoader();
            helper.respondWith(server, [
                helper.getMockResponse("Hello, world!"),
            ]);

            const str = await import('core/str');

            const result = await str.get_string('hello', 'core');
            expect(server.requestCount).to.equal(1);
    ...
    describe('with a LocalStorage cache', () => {
        it('will return a string from the local storage cache', async () => {
            td.replace(helper.getModulePath('core/localstorage'), ({
                get: sinon.fake(() => {
                    return 'cached value';
                }),
