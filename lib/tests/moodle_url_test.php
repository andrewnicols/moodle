...
/**
 * Tests for \core\moodle_url.
 *
 * @package   core
 * @copyright 2018 Andrew Nicols <andrew@nicols.co.uk>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @covers    \core\moodle_url
 */
final class moodle_url_test extends \advanced_testcase {
    ...
    public function test_constructor(): void {
        ...
        $url = new moodle_url('/index.php', ['id' => 2]);
        $this->assertSame($CFG->wwwroot . '/index.php?id=2', $url->out());

        $url = new moodle_url('/index.php', ['id' => 'two']);
        $this->assertSame($CFG->wwwroot . '/index.php?id=two', $url->out());

        $url = new moodle_url('/index.php', ['id' => 1, 'cid' => '2']);
        ...
        $url = new moodle_url('/index.php', ['id' => 2], 'test');
    ...
    public function test_empty_param_values(): void {
        ...
        $url = new moodle_url($strurl, ['id' => 0]);
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'http://moodle.org/course/view.php?id';
        $url = new moodle_url($strurl, ['id' => false]);
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'http://moodle.org/course/view.php?id';
        $url = new moodle_url($strurl, ['id' => null]);
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'http://moodle.org/course/view.php?id';
        $url = new moodle_url($strurl, ['id' => '']);
        $this->assertSame($strurl, $url->out(false));
    ...
    public function test_compare_url(): void {
        $url1 = new moodle_url('index.php', ['var1' => 1, 'var2' => 2]);
        $url2 = new moodle_url('index2.php', ['var1' => 1, 'var2' => 2, 'var3' => 3]);
        ...
        $url2 = new moodle_url('index.php', ['var1' => 1, 'var3' => 3]);

        $this->assertTrue($url1->compare($url2, URL_MATCH_BASE));
        $this->assertFalse($url1->compare($url2, URL_MATCH_PARAMS));
        $this->assertFalse($url1->compare($url2, URL_MATCH_EXACT));

        $url2 = new moodle_url('index.php', ['var1' => 1, 'var2' => 2, 'var3' => 3]);

        $this->assertTrue($url1->compare($url2, URL_MATCH_BASE));
        $this->assertTrue($url1->compare($url2, URL_MATCH_PARAMS));
        $this->assertFalse($url1->compare($url2, URL_MATCH_EXACT));

        $url2 = new moodle_url('index.php', ['var2' => 2, 'var1' => 1]);
