 ...
 * @covers    \core\formatter
 * @coversDefaultClass \core\formatter
 */
class formatter_test extends \advanced_testcase {
    /**
    ...
    public function test_format_string_striptags_cfg(): void {
        ...
        $this->assertSame('x &lt; 1', $formatter->format_string('x < 1'));
        $this->assertSame('x &gt; 1', $formatter->format_string('x > 1'));
        $this->assertSame('x &lt; 1 and x &gt; 0', $formatter->format_string('x < 1 and x > 0'));
    ...
    public function test_format_string_striptags_prop(): void {
        ...
        // Check < and > signs.
        $formatter->set_striptags(false);
        $this->assertSame('x &lt; 1', $formatter->format_string('x < 1'));
        $this->assertSame('x &gt; 1', $formatter->format_string('x > 1'));
        $this->assertSame('x &lt; 1 and x &gt; 0', $formatter->format_string('x < 1 and x > 0'));

        $formatter->set_striptags(true);
        $this->assertSame('x &lt; 1', $formatter->format_string('x < 1'));
        $this->assertSame('x &gt; 1', $formatter->format_string('x > 1'));
        $this->assertSame('x &lt; 1 and x &gt; 0', $formatter->format_string('x < 1 and x > 0'));
    }
    ...
    public function test_format_string_values(
        ...
        $this->assertSame(
            $expected,
            $formatter->format_string($input, ...$options),
        );
    }
    ...
    public function test_format_string_static_caching_with_filters(): void {
        ...
        $cachedresult = $formatter->format_string($rawstring, $striplinks, $options);
