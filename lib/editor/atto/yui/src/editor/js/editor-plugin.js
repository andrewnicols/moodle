...
Y.extend(EditorPlugin, Y.Base, {
    ...
    /**
     * Register shared handlebar helpers.
     */
    registerHandlebarsHelpers: function() {
        Y.Handlebars.registerHelper('startToggleableFieldset', function(options) {
            var legendText = '{{get_string "' + options.hash.key + '" "' + options.hash.component + '"}}';
            if (options.hash.extra) {
                legendText += ' ' + options.hash.extra;
            }

            var collapsedClass = options.hash.collapsed ? ' collapsed' : '';

            return Y.Handlebars.compile(
                '<fieldset class="collapsible ' + collapsedClass + '" id="' + options.hash.id + '" aria-expanded="false">' +
                    '<input name="mform_isexpanded_' + options.hash.id + '" type="hidden">' +
        ...
        Y.Handlebars.registerHelper('endToggleableFieldset', function() {
            return '' +
                    '</div>' +
                '</fieldset>';
        });
