...
import {getTinyMCE, addTinyPremium, baseUrl} from './loader';
...
const getEditorConfiguration = (target, tinyMCE, options, pluginValues) => {
    ...
    if (options.apikey) {
        ...
        instanceConfig.toolbar = addToolbarButton(instanceConfig.toolbar, 'content', 'a11ycheck');
    }
...
export const setupForTarget = async(target, options = {}) => {
    ...
    if (options.apikey) {
        await addTinyPremium(options.apikey);
    }
