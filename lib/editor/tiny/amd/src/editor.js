...
export const setupForTarget = async(target, options = {}) => {
    ...
    // Update the label to point to the editor.
    const textareaLabel = document.querySelector(`label[for="${target.id}"]`);
    const focusEditorHandler = () => {
        editor.focus();
    };
    if (textareaLabel) {
        if (!textareaLabel.id) {
            textareaLabel.setAttribute('id', `${target.id}_label`);
        }
        editor.getContainer().setAttribute('aria-labelledby', textareaLabel.id);

        // Traditionally clicking on a label will cause focus to move to the associated input.
        // This is not natively supported with non-labelable elements so we handle it here.
        textareaLabel.addEventListener('click', focusEditorHandler);
    }
    ...
    editor.on('remove', ({target}) => {
        if (textareaLabel) {
            // Remove the a label event listener that handle focusing on the editor.
            textareaLabel.removeEventListener('click', focusEditorHandler);
        }
