...
export const getSetup = async () => {
    ...
    ] = await getStrings(['imagebuttontitle', 'videobuttontitle', 'imagebuttontitle'].map((key) => ({key, component})));
    ...
    return (editor) => {
        ...
        // Register the Menu Button as a toggle.
        // This means that when highlighted over an existing Media Manager element it will show as toggled on.
        editor.ui.registry.addIcon(mediaManagerIcon, mediaManagerButtonImage.html);
        editor.ui.registry.addToggleButton(mediaManagerButtonName, {
        ...
        editor.ui.registry.addContextMenu(mediaManagerButtonName, {
            update: false,
        });
    };
};
