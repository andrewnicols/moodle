...
/**
   ...
   */
const configureMenu=menu=>{const items=menu.insert.items.split(" ");return items.some(((item,index)=>!!item.match(/(media|video)\b/)&&(items.splice(index+1,0,_common.audioButtonName,_common.videoButtonName,_common.screenButtonName),!0)))?menu.insert.items=items.join(" "):(0,_utils.addMenubarItem)(menu,"insert",`${_common.audioButtonName} ${_common.videoButtonName} ${_common.screenButtonName}`),menu};_exports.configure=instanceConfig=>{return{toolbar:(toolbar=instanceConfig.toolbar,toolbar.map((section=>("content"===section.name&&(section.items.some(((item,index)=>!!item.match(/(media|video)\b/)&&(section.items.splice(index+1,0,_common.audioButtonName,_common.videoContextMenuName),!0)))||section.items.unshift(_common.audioButtonName,_common.videoContextMenuName)),section)))),menu:configureMenu(instanceConfig.menu)};var toolbar}}));
