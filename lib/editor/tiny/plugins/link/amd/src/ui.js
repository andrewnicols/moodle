...
const displayDialogue = async(editor) => {
    ...
    const modalPromises = await ModalFactory.create({
        type: LinkModal.TYPE,
        templateContext: getTemplateContext(editor, data, isUpdating),
        large: false,
    });
    ...
    root.querySelector(Selectors.elements.urlEntry).addEventListener('keyup', () => {
        updateTextToDisplay(currentForm, hasTextToDisplay);
    });
    ...
    root.querySelector(Selectors.elements.urlText).addEventListener('keyup', () => {
        hasTextToDisplay = getTextToDisplayState(currentForm);
    });
};
 ...
 * @returns {Object}
 */
const getTemplateContext = (editor, data, isupdating) => {
 ...
 *
 * @param {Element} currentForm
 * @param {Bool} hasTextToDisplay
 */
const updateTextToDisplay = (currentForm, hasTextToDisplay) => {
 ...
 * @return {Bool}
 */
const getTextToDisplayState = (currentForm) => {
    const urlText = currentForm.querySelector(Selectors.elements.urlText);
