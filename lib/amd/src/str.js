    ...
    const stringPromises = requests.map((request) => {
        ...
        if (cacheKey in promiseCache) {
            ...
            return buildReturn(new Promise((resolve, reject) => {
                ...
                .then((response) => response.strings)
                .then((strings) => {
                    if (!M.str[component]) {
                        M.str[component] = {};
                    }
                    ...
                    Object.entries(strings).forEach(([identifier, value]) => {
                        const match = identifier.match(/^(?<component>[^\/]+)\/(?<stringid>[^\/]+)$/);
                        M.str[match.groups.component][match.groups.stringid] = value;
                        const cacheKey = getCacheKey({
                            lang,
                            component: match.groups.component,
                            key: match.groups.stringid,
                        });
...
export const getComponentStrings = (component, lang = pageLang) => {
    return Fetch.performGet(
        'core',
