...
/**
 * Store a specific tab-index it for later restoration.
 *
 * @method
 * @param {HTMLElement} element
 * @param {Number|undefined} value (-1 or 0)
 */
const _storeUnhideTabIndex = (element, value) => {

    // Store the old tabindex in a data attribute.
    if (value !== undefined) {
        element.dataset.ariaHiddenTabIndex = value;
    } else {
        element.dataset.ariaHiddenTabIndex = '';
    }
    // Ensure the element has no tabindex.
    element.setAttribute('tabindex', -1);
};

/**
 * Update the stored tab index of an element.
 *
 * Ensures aria-hidden elements do not contain focusable elements (wcag143). This means
 * that hidden elements cannot use tabindex="0". When a new element is create inside
 * a hidden element, this method can be used to define which tab index will have in case
 * it is unhidden.
 *
 * @method
 * @param {HTMLElement} target
 * @param {Number|undefined} value (-1 or 0)
 */
export const storeUnhideTabIndex = (target, value) => {
    getList(target).forEach(element => _storeUnhideTabIndex(element, value));
};
