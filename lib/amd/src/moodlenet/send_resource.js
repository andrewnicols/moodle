    ...
    document.addEventListener('click', async(e) => {
        ...
        if (shareAction) {
            ...
            if (type == TYPE_ACTIVITY) {
                try {
                    handleServiceInformation(cmId, shareType);
                } catch (e) {
                    displayException(e);
                }
            }
 ...
 */
const handleServiceInformation = async(cmId, shareType) => {
    const data = await MoodleNetService.getActivityInformation(cmId);
    if (data.status) {
        siteSupportUrl = data.supportpageurl;
        issuerId = data.issuerid;
        currentModal = await SendActivityModal.create({
            large: true,
            templateContext: {
