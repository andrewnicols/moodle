...
const displayConfirmation = (source, type) => {
    ...
    if (`${type}Type` in source.dataset && source.dataset[`${type}Type`] === 'delete') {
        confirmationPromise = deleteCancelPromise(
            getModalString(source.dataset, type, 'Title', ['confirmation', 'admin']),
 ...
 */
const openPopup = (dataset) => {
    const getPopupName = (popupName) => {
    ...
    const isFullScreen = JSON.parse(dataset.popupFullscreen) || false;
    const options = JSON.parse(dataset.popupOptions) || {};
    ...
    const windowobj = window.open(
        getFullUrl(JSON.parse(dataset.popupUrl)),
        getPopupName(JSON.parse(dataset.popupName)),
...
const registerConfirmationListeners = () => {
    document.addEventListener('click', e => {
        ...
        const popupRequest = e.target.closest('[data-popup]');
        if (popupRequest) {
            e.preventDefault();
            openPopup(popupRequest.dataset);
        }
    });
};
