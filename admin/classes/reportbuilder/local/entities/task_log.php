...
class task_log extends base {
    ...
    protected function get_default_entity_title(): lang_string {
        return new lang_string('entitytasklog', 'core_admin');
    }
    ...
    protected function get_all_columns(): array {
        ...
        $columns[] = (new column(
            'type',
            new lang_string('tasktype', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_TEXT)
            ...
            ->add_callback(static function($value): string {
                if (\core\task\database_logger::TYPE_SCHEDULED === (int) $value) {
                    return get_string('task_type:scheduled', 'core_admin');
                }
                return get_string('task_type:adhoc', 'core_admin');
            });

        // Start time column.
        $columns[] = (new column(
            'starttime',
            new lang_string('task_starttime', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
        ...
        $columns[] = (new column(
            'endtime',
            new lang_string('task_endtime', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_TIMESTAMP)
        ...
        $columns[] = (new column(
            'duration',
            new lang_string('task_duration', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
        ...
        $columns[] = (new column(
            'hostname',
            new lang_string('hostname', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_TEXT)
        ...
        $columns[] = (new column(
            'pid',
            new lang_string('pid', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
        ...
        $columns[] = (new column(
            'database',
            new lang_string('task_dbstats', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_INTEGER)
            ...
            ->add_callback(static function(int $value, stdClass $row): string {
                $output = '';
                $output .= \html_writer::div(get_string('task_stats:dbreads', 'core_admin', $row->dbreads));
                $output .= \html_writer::div(get_string('task_stats:dbwrites', 'core_admin', $row->dbwrites));
                return $output;
            })
            // Although this is an integer column, it doesn't make sense to perform numeric aggregation on it.
            ->set_disabled_aggregation(['avg', 'count', 'countdistinct', 'max', 'min', 'sum']);
        ...
        $columns[] = (new column(
            'dbreads',
            new lang_string('task_dbreads', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_INTEGER)
        ...
        $columns[] = (new column(
            'dbwrites',
            new lang_string('task_dbwrites', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_INTEGER)
        ...
        $columns[] = (new column(
            'result',
            new lang_string('task_result', 'core_admin'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ...
            ->add_callback(static function(bool $success): string {
                if (!$success) {
                    return get_string('task_result:failed', 'core_admin');
                }
    ...
    protected function get_all_filters(): array {
        ...
        $filters[] = (new filter(
            ...
            ->add_joins($this->get_joins())
            ->set_options([
                \core\task\database_logger::TYPE_ADHOC => new lang_string('task_type:adhoc', 'core_admin'),
                \core\task\database_logger::TYPE_SCHEDULED => new lang_string('task_type:scheduled', 'core_admin'),
            ]);
        ...
        $filters[] = (new filter(
            text::class,
            'output',
            new lang_string('task_logoutput', 'core_admin'),
            $this->get_entity_name(),
        ...
        $filters[] = (new filter(
            ...
            ->set_options([
                ...
                self::FAILED => get_string('task_result:failed', 'core_admin'),
            ]);
