...
if (!empty($action) and confirm_sesskey()) {
    ...
    if (!has_capability('moodle/user:delete', $sitecontext)) {
        throw new \moodle_exception('nomodifyacl', 'core_mnet');
    }

    // fetch the record in question
    $id = required_param('id', PARAM_INT);
    if (!$idrec = $DB->get_record('mnet_sso_access_control', array('id'=>$id))) {
        throw new \moodle_exception('recordnoexists', 'core_mnet', "$CFG->wwwroot/$CFG->admin/mnet/access_control.php");
    }
    ...
    switch ($action) {
        ...
        case "delete":
            ...
            redirect('access_control.php', get_string('deleteuserrecord', 'core_mnet', array('user'=>$idrec->username, 'host'=>$mnethosts[$idrec->mnet_host_id])));
            break;
        ...
        case "acl":
            ...
            if ($accessctrl != 'allow' and $accessctrl != 'deny') {
                throw new \moodle_exception('invalidaccessparam', 'core_mnet', "$CFG->wwwroot/$CFG->admin/mnet/access_control.php");
            }

            if (mnet_update_sso_access_control($idrec->username, $idrec->mnet_host_id, $accessctrl)) {
                if ($accessctrl == 'allow') {
                    redirect('access_control.php', get_string('ssl_acl_allow','core_mnet', array('user' => $idrec->username,
                        'host' => $mnethosts[$idrec->mnet_host_id])));
                } else if ($accessctrl == 'deny') {
                    redirect('access_control.php', get_string('ssl_acl_deny','core_mnet', array('user' => $idrec->username,
                        'host' => $mnethosts[$idrec->mnet_host_id])));
                }
            }
            break;
        ...
        default:
            throw new \moodle_exception('invalidactionparam', 'core_mnet', "$CFG->wwwroot/$CFG->admin/mnet/access_control.php");
    }
}
...
if ($form = data_submitted() and confirm_sesskey()) {
    ...
    if (!has_capability('moodle/user:delete', $sitecontext)) {
        throw new \moodle_exception('nomodifyacl', 'core_mnet', "$CFG->wwwroot/$CFG->admin/mnet/access_control.php");
    }
    if (empty($form->username)) {
        $formerror['username'] = get_string('enterausername','core_mnet');
    }
    if (empty($form->mnet_host_id)) {
        $formerror['mnet_host_id'] = get_string('selectahost','core_mnet');
    }
    if (empty($form->accessctrl)) {
        $formerror['accessctrl'] = get_string('selectaccesslevel','core_mnet'); ;
    }
            ...
            if (!empty($username)) {
                if (mnet_update_sso_access_control($username, $form->mnet_host_id, $form->accessctrl)) {
                    if ($form->accessctrl == 'allow') {
                        redirect('access_control.php', get_string('ssl_acl_allow','core_mnet', array('user'=>$username, 'host'=>$mnethosts[$form->mnet_host_id])));
                    } elseif ($form->accessctrl == 'deny') {
                        redirect('access_control.php', get_string('ssl_acl_deny','core_mnet', array('user'=>$username, 'host'=>$mnethosts[$form->mnet_host_id])));
                    }
                }
            }
        }
...
// Explain
echo $OUTPUT->box(get_string('ssoacldescr','core_mnet'));
// Are the needed bits enabled?
$warn = '';
if (empty($CFG->mnet_dispatcher_mode) || $CFG->mnet_dispatcher_mode !== 'strict') {
    $warn = '<p>' . get_string('mnetdisabled','core_mnet') .'</p>';
}

if (!is_enabled_auth('mnet')) {
    $warn .= '<p>' .  get_string('authmnetdisabled','core_mnet').'</p>';
}

if (!empty($warn)) {
    $warn = '<p>' .  get_string('ssoaclneeds','core_mnet').'</p>' . $warn;
...
$string = array('username'     => get_string('username'),
                'mnet_host_id' => get_string('remotehost', 'core_mnet'),
                'access'       => get_string('accesslevel', 'core_mnet'),
...
if (!$acl) {
    echo $OUTPUT->heading(get_string('noaclentries','core_mnet'));
    $table = NULL;
} else {
    ...
    foreach ($acl as $aclrecord) {
        if ($aclrecord->accessctrl == 'allow') {
            $accesscolumn = get_string('allow', 'core_mnet')
                . " (<a href=\"?id={$aclrecord->id}&amp;action=acl&amp;accessctrl=deny&amp;sesskey=".sesskey()."\">"
                . get_string('deny', 'core_mnet') . "</a>)";
        } else {
            $accesscolumn = get_string('deny', 'core_mnet')
                . " (<a href=\"?id={$aclrecord->id}&amp;action=acl&amp;accessctrl=allow&amp;sesskey=".sesskey()."\">"
                . get_string('allow', 'core_mnet') . "</a>)";
        }
...
echo " " . html_writer::label(get_string('remotehost', 'core_mnet'), 'menumnet_host_id') . ":\n";
...
echo " " . html_writer::label(get_string('accesslevel', 'core_mnet'), 'menuaccessctrl') . ":\n";
if (!empty($formerror['accessctrl'])) {
    echo '<span class="error"> * </span>';
}
$accessmenu['allow'] = get_string('allow', 'core_mnet');
$accessmenu['deny'] = get_string('deny', 'core_mnet');
echo html_writer::select($accessmenu, 'accessctrl');

// submit button
echo '<input type="submit" value="' . get_string('addtoacl', 'core_mnet') . '" />';
echo "</form></div>\n";
