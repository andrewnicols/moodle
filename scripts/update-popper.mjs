...
/**
 * Script to update the popperjs bundles.
 *
...
const popperOutputDir = path.join(outputdir, '@popperjs');

// Bundle config.
const TARGET = "es2022";
const POPPER_VERSION = getPackageVersion('@popperjs/core');

// Bundles to download.
const bundles = [
  { packageName: "@popperjs/core", version: POPPER_VERSION, fileName: "core" },
].map((bundle) => ({
  ...bundle,
...
/**
 * Update the Popper bundles by downloading them from esm.sh and saving them in the lib folder.
 * Also updates the version in thirdpartylibs.xml.
 * The version is read from the npm-shrinkwrap.json file.
 */
async function init() {
  console.log(chalk.blue.bold.underline('Updating Popper bundles to version %s using esm.sh'), POPPER_VERSION);

  console.log(chalk.blue('Removing old bundles...'));
  fs.removeSync(popperOutputDir, { recursive: true, force: true });
    ...
    await download(bundle.url, filePath, (filePath) => {
      ...
      bundles.forEach((bundle) => {
        if (bundle.packageName === bundle.fileName) {
          // For the main popperjs bundle, we need to replace the import paths to remove the version number.
          ...
          // For example, the import path in the bundle might be "/popperjs@2.11.8/es2022/popper.js", but we want it to be "popperjs".
  ...
  createPackageReadme(popperOutputDir, 'popperjs');

  // Update the version in thirdpartylibs.xml.
  console.log(chalk.green(`Updating thirdpartylibs.xml files ✓`));
  updateThirdPartyLibsXml(path.join(rootDir, 'lib'), 'js/bundles/@popperjs', 'popper', POPPER_VERSION);
