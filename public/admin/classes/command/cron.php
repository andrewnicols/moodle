...
class cron extends Command {
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int {
        ...
        if ($input->getOption('disable-wait')) {
            return $this->disable_after_wait($input, $output);
        }

        if (!$this->taskmanager->is_cron_enabled() && !$input->getOption('force')) {
            $output->writeln('<error>Cron is currently disabled. Please enable it to run tasks, or pass the --force option</error>');
            return 1;
        }
    ...
    private function disable_after_wait(InputInterface $input, OutputInterface $output): int {
        ...
        while (true) {
            ...
            if (time() >= $endtime) {
                $progressbar->finish();
                $output->writeln('');
                $output->writeln('<error>Wait time (' . format_time($waitsec) . ') elapsed, but ' . count($tasks) . ' task(s) still running.</error>');
