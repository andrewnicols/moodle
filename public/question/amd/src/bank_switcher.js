...
export default class BankSwitcher {
     ...
     */
    async show(modal, courseId, currentCmId = null, activityCmId = null) {
        if (!courseId) {
        ...
        const switcherEndpoint = ['bank', courseId, 'switcher'];
        const params = {};
        if (activityCmId) {
            params.currentcmid = activityCmId;
        }
        const switcher = await Fetch.performGet('question', switcherEndpoint.join('/'), {params});
        modal.setBody(
            Templates.render('core_question/switch_question_bank', await switcher.json())
        );
