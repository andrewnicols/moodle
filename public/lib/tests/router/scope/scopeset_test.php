...
final class scopeset_test extends \advanced_testcase {
    ...
    /**
     * A scope set with no required scopes is trivially satisfied.
     */
    #[RunInSeparateProcess]
    public function test_empty_scopeset_is_always_satisfied(): void {
        $scopeset = new scopeset();

        $this->assertSame([], $scopeset->requiredscopes);
        $this->assertTrue($scopeset->is_satisfied_by([]));
        $this->assertTrue($scopeset->is_satisfied_by(['anything:else']));
    }
