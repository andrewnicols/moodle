...
final class import_map_test extends \advanced_testcase {
    ...
    public function test_resolve_appends_suffix_when_not_present(): void {
        ...
        $map = new import_map();
        $map->set_default_loader(new \core\url('https://example.com/'));
        $map->add_import('test/', path: 'testpath', allowedsuffixes: ['.js', '.js.map']);
    ...
    /**
     * get_path_for_script() appends /index when the resolved path is a directory.
     */
    public function test_resolve_appends_index_for_directory(): void {
        global $CFG;

        $tempdir = make_request_directory();
        mkdir("{$tempdir}/testpath/subdir", 0777, true);
        file_put_contents("{$tempdir}/testpath/subdir/index.js", '// index');
        ...
        $result = $map->get_path_for_script(1, 'test/subdir');
        $this->assertEquals("{$tempdir}/testpath/subdir/index.js", $result);
    }
    ...
    public function test_resolve_appends_suffix_when_suffix_present_but_file_missing(): void {
        ...
        // module.js does not exist on disk, so even though path ends with .js, suffix is still appended.
        $result = $map->get_path_for_script(1, 'test/module.js');
