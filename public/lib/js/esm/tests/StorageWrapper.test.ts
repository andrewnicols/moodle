...
/**
 * Tests for the StorageWrapper ESM module.
 *
 * @copyright  Andrew Lyons <andrew@nicols.co.uk>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

import StorageWrapper from '@moodle/lms/core/StorageWrapper';
...
describe('StorageWrapper', () => {
    describe('hashString', () => {
        it('returns 0 for an empty string', () => {
            expect(StorageWrapper.hashString('')).toBe(0);
        });

        it('returns a consistent hash for the same input', () => {
            const hash1 = StorageWrapper.hashString('hello');
            const hash2 = StorageWrapper.hashString('hello');
            expect(hash1).toBe(hash2);
        });

        it('returns different hashes for different inputs', () => {
            const hash1 = StorageWrapper.hashString('hello');
            const hash2 = StorageWrapper.hashString('world');
    ...
    describe('when jsrev is a positive value (caching enabled)', () => {
        ...
        it('prefixes keys to avoid collisions', () => {
            ...
            const prefix = `${StorageWrapper.hashString(hashSource)}/`;
    ...
    describe('when storage throws on setItem', () => {
        ...
        it('set returns false when storage is full', () => {
            ...
            // Construct after overriding setItem would fail during validation,
            // so override after construction.
            const realSetItem = createMockStorage().setItem.bind(createMockStorage());
            const mockStorage = createMockStorage();
            const wrapper = new StorageWrapper(mockStorage);
