...
describe('core/utils', () => {
    ...
    describe('debounce', () => {
        ...
        it('registers and resolves pending when pending option is enabled', async () => {
            ...
            expect((globalThis as any).pendingStack).toContain('core/utils:debounce');
            expect((globalThis as any).completeStack).toHaveLength(0);
            ...
            expect(fn).toHaveBeenCalledTimes(1);
            expect((globalThis as any).completeStack).toContain('core/utils:debounce');
        });
    });
