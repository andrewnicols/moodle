...
/** Minimal shape of a jQuery deferred returned by core/ajax.call(). */
type JQueryPromiseLike<T> = {
    then(
...
type AmdAjaxModule = {
    call(
        ...
        timeout?: number,
        cachekey?: number,
    ): Array<JQueryPromiseLike<unknown>>;
};


let loadPromise: Promise<AmdAjaxModule> | null = null;

function getAjaxModule(): Promise<AmdAjaxModule> {
    if (!loadPromise) {
        loadPromise = requireAsync<AmdAjaxModule>("core/ajax");
    }
    return loadPromise;
}

/** Converts a jQuery-compatible promise into a native Promise. */
function toNativePromise<T>(jqPromise: JQueryPromiseLike<unknown>): Promise<T> {
...
export async function fetchMany<T = unknown>(
    ...
    const ajax = await getAjaxModule();
    const nativePromises = ajax
        .call(requests, isAsync, loginrequired, nosessionupdate)
