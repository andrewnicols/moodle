...
class clamav extends \core\feature\antivirus_feature {
    ...
    /** @var int Default socket data stream chunk size (32Mb: 32 * 1024 * 1024) */
    private const SOCKET_CHUNKSIZE = 32 * 1024 * 1024;
    ...
    public function is_configured() {
        ...
        if ($this->get_config('runningmethod') === 'tcpsocket') {
    ...
    public function scan_file($file, $filename) {
        ...
        $maxtries = get_config('plugin_clamav', 'tries');
        $tries = 0;
        do {
        ...
        $notice = get_string('tries_notice', 'plugin_clamav', ['tries' => $tries, 'notice' => $this->get_scanning_notice()]);
    ...
    public function scan_data($data) {
        ...
        // if not, use default process.
        $runningmethod = $this->get_config('runningmethod');
        if (in_array($runningmethod, ['unixsocket', 'tcpsocket'])) {
    ...
    private function get_clam_error_code($returncode) {
        ...
        return get_string('unknownerror', 'plugin_clamav');
    }
    ...
    public function scan_file_execute_commandline($file) {
        ...
        if (!file_exists($pathtoclam) || !is_executable($pathtoclam)) {
            ...
            $notice = get_string('invalidpathtoclam', 'plugin_clamav', $pathtoclam);
            $this->set_scanning_notice($notice);
            return self::SCAN_RESULT_ERROR;
        }
        ...
        if ($return > self::SCAN_RESULT_FOUND) {
            $notice = get_string('clamfailed', 'plugin_clamav', $this->get_clam_error_code($return));
            $notice .= "\n\n" . implode("\n", $output);
            $this->set_scanning_notice($notice);
            return self::SCAN_RESULT_ERROR;
        } else {
    ...
    public function scan_file_execute_socket($file, $type) {
        ...
        if (!$socket) {
            ...
            $notice = get_string('errorcantopensocket', 'plugin_clamav', "$errstr ($errno)");
            $this->set_scanning_notice($notice);
            return self::SCAN_RESULT_ERROR;
        } else {
            if ($type == "unixsocket") {
                ...
                while (!feof($fhandle)) {
                    ...
                    $chunk = fread($fhandle, self::SOCKET_CHUNKSIZE);
    ...
    public function scan_data_execute_socket($data, $type) {
        ...
        $socket = stream_socket_client($socketurl, $errno, $errstr, self::SOCKET_TIMEOUT);
        if (!$socket) {
            // Can't open socket for some reason, notify admins.
            $notice = get_string('errorcantopensocket', 'plugin_clamav', "$errstr ($errno)");
            $this->set_scanning_notice($notice);
            ...
            // this is to avoid unexpected newline characters on different systems.
            fwrite($socket, "nINSTREAM\n");
            // Send data in chunks of SOCKET_CHUNKSIZE size.
            while (strlen($data) > 0) {
                $chunk = substr($data, 0, self::SOCKET_CHUNKSIZE);
                $data = substr($data, self::SOCKET_CHUNKSIZE);
                $size = pack('N', strlen($chunk));
                fwrite($socket, $size);
                fwrite($socket, $chunk);
            }
    ...
    private function parse_socket_response($output) {
        ...
        if ($message === 'OK') {
            ...
            if ($status === 'FOUND') {
                ...
                $notice = get_string('clamfailed', 'plugin_clamav', $this->get_clam_error_code(2));
                $notice .= "\n\n" . $output;
                $this->set_scanning_notice($notice);
                return self::SCAN_RESULT_ERROR;
            }
