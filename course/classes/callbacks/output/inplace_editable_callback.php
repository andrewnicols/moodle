...
namespace core_course\callbacks\output;
    ...
    inplace_editable_interface
{
    ...
    public function execute_inplace_editable(inplace_editable_object $callback): void {
        match ($callback->itemtype) {
            'activityname' => $this->inplace_editable_activityname($callback),
            default => null,
        };
    }

    /**
     * Inplace editable callback for activity name.
     *
     * @param inplace_editable_object $callback
     */
    private function inplace_editable_activityname(inplace_editable_object $callback): void {
        $callback->set_renderable(
            \core_courseformat\output\local\content\cm\title::update($callback->itemid, $callback->newvalue),
        );
    }
