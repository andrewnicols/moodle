...
class core_course_external extends external_api {
    ...
    public static function get_course_contents($courseid, $options = []): array {
        ...
        $helper->set_include_modules(empty($filters['excludemodules']));
        $helper->set_include_contents(empty($filters['excludecontents']));
        $helper->set_include_stealth_modules(!empty($filters['includestealthmodules']));

        if (!empty($filters['modname'])) {
            $helper->set_activity_name_filter($filters['modname']);
        }

        if (!empty($filters['modid'])) {
            $helper->set_modid_filter($filters['modid']);
        }

        if (!empty($filters['cmid'])) {
            $helper->set_cmid_filter($filters['cmid']);
        }

        // Security checks.
        $context = $helper->get_course_context();
        try {
        ...
        if (!empty($filters['sectionid'])) {
            return $helper->get_fileinfo_for_section_id($filters['sectionid']);
        } else if (!empty($filters['sectionnumber'])) {
            return $helper->get_fileinfo_for_section_number($filters['sectionnumber']);
        } else {
            return $helper->get_fileinfo_for_all_sections();
        }
    }
