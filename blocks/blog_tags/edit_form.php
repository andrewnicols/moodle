...
class block_blog_tags_edit_form extends block_edit_form {
    protected function specific_definition($mform) {
        ...
        $mform->setDefault('config_title', get_string('blogtags', 'core_blog'));
        ...
        $mform->addElement('select', 'config_timewithin', get_string('timewithin', 'core_blog'), $timewithin);
        $mform->setDefault('config_timewithin', BLOCK_BLOG_TAGS_DEFAULTTIMEWITHIN);

        $sort = array(
            'name' => get_string('tagtext', 'core_blog'),
            'id'   => get_string('tagdatelastused', 'core_blog'),
        );
        $mform->addElement('select', 'config_sort', get_string('tagsort', 'core_blog'), $sort);
