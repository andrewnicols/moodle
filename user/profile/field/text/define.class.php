...
class profile_define_text extends profile_define_base {
    ...
    public function define_form_specific($form) {
        ...
        $form->addElement('text', 'param1', get_string('profilefieldsize', 'core_admin'), 'size="6"');
        $form->setDefault('param1', 30);
        $form->setType('param1', PARAM_INT);

        // Param 2 for text type is the maxlength of the field.
        $form->addElement('text', 'param2', get_string('profilefieldmaxlength', 'core_admin'), 'size="6"');
        ...
        $form->addElement('selectyesno', 'param3', get_string('profilefieldispassword', 'core_admin'));
        $form->setDefault('param3', 0); // Defaults to 'no'.
        $form->setType('param3', PARAM_INT);

        // Param 4 for text type contains a link.
        $form->addElement('text', 'param4', get_string('profilefieldlink', 'core_admin'));
        $form->setType('param4', PARAM_URL);
        $form->addHelpButton('param4', 'profilefieldlink', 'admin');

        // Param 5 for text type contains link target.
        $targetoptions = array( ''       => get_string('linktargetnone', 'core_editor'),
                                '_blank' => get_string('linktargetblank', 'core_editor'),
                                '_self'  => get_string('linktargetself', 'core_editor'),
                                '_top'   => get_string('linktargettop', 'core_editor')
                              );
        $form->addElement('select', 'param5', get_string('profilefieldlinktarget', 'core_admin'), $targetoptions);
